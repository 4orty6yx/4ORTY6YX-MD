...
zokou({nomCom:"bot",reaction:"📡",categorie:"🔮Artificial Intelligence"},async(dest,zk,commandeOptions)=>{
...
.then(data => {
  ...
  traduire(botResponse, { to: 'en' })
    ...
    .catch(error => {
      ...
      repondre('Error when translating into kiswahili');
    });
})
.catch(error => {
  ...
  zokou({ nomCom: "dalle", reaction: "📡", categorie: "🔮Artificial Intelligence" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*powered by 4ORTY6YX-OFFICIAL-MD*';
  ...
  zokou({ nomCom: "ai", reaction: "📡", categorie: "🔮Artificial Intelligence" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
