...
zokou({nomCom : "instagram" , categorie : "📥Downloader"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader powered by *4ORTY6YX-OFFICIAL-MD*",gifPlayback : false },{quoted : ms}) 
    }
    else {
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       ...
       zk.sendMessage(dest, { video: { url: result.hd  }, caption: 'facebook video downloader powered by *4ORTY6YX-OFFICIAL-MD*' }, { quoted: ms });
...
async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       zk.sendMessage(dest,{image : { url : result.thumbnail}, caption : caption},{quoted : ms}) ;
       zk.sendMessage(dest, { video: { url: result.sd  }, caption: 'facebook video downloader powered by *4ORTY6YX-OFFICIAL-MD*' }, { quoted: ms });
